...
COLOR LAB — Gamut Explorer. The active app is **`fe/`** (SvelteKit 2 + Svelte 5 runes + TypeScript, WebGL2 renderer). `fe.old/` is a deprecated Svelte 3 / Rollup / Cypress version — ignore it unless explicitly asked. `_docs/` holds planning/design notes. All commands below run from `fe/`.
