...
- **Concept:** Gamut map is the current terminal ramp stage: it brings any out-of-gamut stops into the configured ramp output intent. Today the UI still exposes this as a ramp-only OOG policy; roadmap direction is for ramp output intent to follow the Active gamut and for constraints in Interpolate/Expand to do more of the repair work. **Clip (clamp)** hard-clips each RGB channel at 0 and 1 (fast, may shift hue). **Preserve chroma** reduces chroma in Oklch until the stop is in-gamut (preserves hue and lightness). **None** passes OOG stops through unchanged. This stage is separate from Display gamut preview and from CVD.
...
- **Concept:** With more than one source list, each list runs independently through Interpolate → Place → Gamut map and produces its own output. Current builds still share many pipeline settings across lists, but roadmap direction is for each source list to own its own interpolation, placement, extension, and constraint settings. That will make each list a true pipeline instance. When Expand is off, the palette grid shows all lists' ramps as parallel rows; when Expand is on, the Expand row generator applies to the combined set.
